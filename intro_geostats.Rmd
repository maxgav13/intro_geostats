...
Con el modelo ajustado (Tabla \@ref(tab:ajuste-tab)) se puede calcular un error del ajuste inicial ($RMSE=`r apa(fit.rmse,3)`$), pero es más confiable el que se obtiene usando la validación cruzada, ya que el obtenido acá es un valor optimista. Lo anterior se da puesto que se calcula con respecto a los datos que se realizó el ajuste y esto simpre va a resultar en error menor que cuando se usa el modelo en datos no observados, que es lo que se quiere [@kuhn2013; @witten2011].
...
Si recordamos el error del ajuste inicial sobre los datos que se realizó el ajuste fue `r apa(fit.rmse,3)`, que como se puede observar es mucho menor al $RMSE$ de la validación cruzada `r apa(xval.rmse,3)` de ahí que se le definiera como optimista.
